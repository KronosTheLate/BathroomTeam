...
##
Ns = [round(Int, 100*1.2^i) for i in 0:10]
...
let
    fig, ax, plt = lines(dts[mask_convergence], errors[mask_convergence])
    ax.xlabel = "Temporal stepsize"
...
# u0 = ComplexF64[exp(-0.5(x-4)^2) for x in p.xs]
# u0 = ComplexF64[exp(  -2(x-4)^2) for x in p.xs]

function leapfrog_integration(system_func, initial_values, t_start, t_end, step_size )
    ...
    for i in 2:N_steps
        ...
        #print(dx)
        #print(x_vect)
        x_vect[1:2] +=@. (dx[1:2] * (t_vector[i] - t_vector[i-1]))
        #x_vect[2] += dx[2] * (t_vector[i] - t_vector[i-1])
        dx = system_func(x_vect)
        x_vect[3:4] +=@. (dx[3:4] * (t_vector[i] - t_vector[i-1]))
        x_solved[:,i] = x_vect
